    ...
    if "degree" in initializer:
        ...
        if "degree_bucket" in initializer:
            ...
            if initializer == "degree_bucket_range":
                ...
                for i in range(bucket_size, 101, bucket_size):
                    boundaries.append(np.percentile(node_degrees, i, interpolation="higher"))
        ...
        try:
            if initializer == "eigen":
                ...
                v = np.load("brazil-airports/brazil-airports_eigenvector_degree_normalized.npy")
            print(v.shape)
        except:
            ...
            if initializer == "eigen":
                np.save("brazil-airports/brazil-airports_eigenvector", v[:1000])
            else:
                np.save("brazil-airports/brazil-airports_eigenvector_degree_normalized", v[:1000])
        # print(v)
        feat_data = np.zeros((num_nodes, feature_dim))
...
def load_usa_airport(feature_dim, initializer="None"):
    ...
    if "degree" in initializer:
        ...
        if "degree_bucket" in initializer:
            ...
            if initializer == "degree_bucket_range":
                bucket_num = 20
                bucket_size = int(241 / bucket_num)
                boundaries = np.arange(bucket_size, 241, bucket_size)
            elif initializer == "degree_bucket_distribution":
                bucket_num = 10
                bucket_size = int(100 / bucket_num)
                for i in range(bucket_size, 101, bucket_size):
        ...
        try:
            if initializer == "eigen":
                ...
                v = np.load("usa-airport/usa-airport_eigenvector_degree_normalized.npy")
            print(v.shape)
        except:
            adj_matrix = nx.to_numpy_array(G)
            # normalize adjacency matrix with degree
            ...
            if initializer == "eigen":
                ...
                np.save("usa-airport/usa-airport_eigenvector_degree_normalized", v[:1000])
        # print(v)
        feat_data = np.zeros((num_nodes, feature_dim))
        # assert(feature_dim <= 1000)
        for i in range(num_nodes):
...
def load_europe_airport(feature_dim, initializer="None"):
    ...
    if "degree" in initializer:
        ...
        if "degree_bucket" in initializer:
            ...
            if initializer == "degree_bucket_range":
                bucket_num = 20
                bucket_size = int(221 / bucket_num)
                boundaries = np.arange(bucket_size, 221, bucket_size)
            elif initializer == "degree_bucket_distribution":
                bucket_num = 10
                bucket_size = int(100 / bucket_num)
                for i in range(bucket_size, 101, bucket_size):
                    boundaries.append(np.percentile(node_degrees, i))
        ...
        try:
            if initializer == "eigen":
                v = np.load("europe-airports/pubmed_eigenvector.npy")
            else:
                v = np.load("europe-airports/pubmed_eigenvector_degree_normalized.npy")
            print(v.shape)
        except:
            adj_matrix = nx.to_numpy_array(G)
            # normalize adjacency matrix with degree
            ...
            # only save top 1000 eigenvectors
            if initializer == "eigen":
                np.save("europe-airports/pubmed_eigenvector", v[:1000])
            else:
                np.save("europe-airports/pubmed_eigenvector_degree_normalized", v[:1000])
        # print(v)
        feat_data = np.zeros((num_nodes, feature_dim))
        # assert(feature_dim <= 1000)
        for i in range(num_nodes):
    ...
    if initializer == "node_degree":
        ...
        try:
            ...
            if initializer == "eigen":
                ...
                np.save("citeseer/citeseer_eigenvector_degree_normalized", v[:1000])
...
def run_model(dataset, initializer, seed, epochs, batch_size=128, feature_dim=100, identity_dim=50, lr=0.01, agg_type="mean"):
    ...
    enc1_num_samples_map = {"cora": 5, "pubmed": 10, "citeseer": 5, "usa-airport": 30, "brazil-airport": 30, "europe-airport": 15}
    enc2_num_samples_map = {"cora": 5, "pubmed": 25, "citeseer": 5, "usa-airport": 30, "brazil-airport": 30, "europe-airport": 15}
    ...
    if agg_type == "sum":
        ...
        agg2 = SumAggregator(lambda nodes : enc1(nodes).t(), num_nodes=num_nodes, cuda=False)
        ...
        agg2 = MeanAggregator(lambda nodes : enc1(nodes).t(), num_nodes=num_nodes, cuda=False)
        enc2 = Encoder(lambda nodes : enc1(nodes).t(), enc1.embed_dim, enc2_dim, adj_lists, agg2,
    ...
    with open(f"results/{dataset}/{dataset}_{agg_type}_{initializer}_{feature_dim}.csv", mode="a") as csv_file:
        writer = csv.writer(csv_file, delimiter=",")
        micro = f1_score(labels[val_idx], val_output.data.numpy().argmax(axis=1), average="micro")
        macro = f1_score(labels[val_idx], val_output.data.numpy().argmax(axis=1), average="macro")
...
def main():
    ...
    run_model(dataset, initializer, seed, epochs, feature_dim=feature_dim, identity_dim=identity_dim, lr=lr, agg_type=agg_type)
